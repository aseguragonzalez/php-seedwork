...
- Construct Commands/Queries so they validate themselves on creation, then dispatch them and let the bus stack handle transactions and domain event coordination.
...
- Use separate outbox storage with independent lifecycles for integration events and background tasks.
...
- On transient failure (infrastructure error): does not acknowledge — the broker retries according to its retry policy.
- On domain failure (`\DomainException` → `Result::failed()`): acknowledge the message (to avoid infinite retries) and route to DLQ or alert for investigation.
...
|---|---|---|---|
| **Polling** | `SELECT … FOR UPDATE SKIP LOCKED` at a fixed interval | Seconds | None on DBs that support row-level locking + `SKIP LOCKED` |
...
> **Polling compatibility.** `FOR UPDATE SKIP LOCKED` is not universal SQL. It is suitable on databases/versions that support it (for example, PostgreSQL and MySQL 8+ with the appropriate engine). On other databases, use a different claiming strategy such as optimistic row claiming with a status transition, lease/lock columns, or a database-native queue/change-feed mechanism.
...
**Key points**
- Use the event or task `id` as the deduplication key — it is a stable string identifier (UUID is the recommended format) generated at event creation time.
...
**Don't**
- Retry on `\DomainException` — it is a deterministic failure.
...
- **`causationId`** — the ID of the direct parent event or task that caused this one. Changes at each step. Answers: *what immediately triggered this?*

> **Where these fields live.** The seedwork `Command` and `DomainEvent` base classes do **not** carry `correlationId` or `causationId`. These fields appear on `IntegrationEvent` and `BackgroundTask` — the types that cross a process or time boundary. `correlationId` is a cross-cutting concern held in a service-defined `RequestContext`, initialised at the entry point and injected into `DomainEventHandler` implementations that construct outbound messages.

### 4.2 Propagation Rules

```
HTTP Request → RequestContext.correlationId = X
  └── Command dispatched               (no correlationId/causationId fields)
        └── DomainEvent emitted        (id = Y; no correlationId/causationId fields)
              └── IntegrationEvent published  (correlationId = X ← RequestContext, causationId = Y)
                    └── BackgroundTask scheduled  (correlationId = X ← RequestContext, causationId = Y)
```

**Rules**
- `correlationId` is set once at the entry point and stored in `RequestContext`. It never changes within a chain. When constructing `IntegrationEvent` or `BackgroundTask`, read it from `RequestContext`.
- `causationId` is the `id` of the domain event that triggered the outbound message. Set it to `domainEvent.id` inside the `DomainEventHandler`.
- If there is no parent (e.g. a cron job or a DLQ re-queue), generate a new `correlationId` and initialise `RequestContext` with it.

### 4.3 Minimum Recommended Fields

**Integration events** — base class provides `id`, `type`, `version`, `aggregateId`, `payload`, `correlationId`, `occurredAt`, `causationId?`, `metadata?`. Add `source` to your concrete subclass.

| Field | Type | Description |
|---|---|---|
| `id` | string | Unique identifier (auto-generated by `IntegrationEvent`; use UUID) |
| `correlationId` | string | Originating operation identifier (UUID recommended) |
| `causationId` | string | Direct parent identifier (UUID recommended) |
| `occurredAt` | ISO-8601 | When the fact occurred (UTC) |
| `type` | string | Event type discriminator |
| `version` | string | Schema version (`"1"`, `"2"`) |
| `source` | string | Service that emitted this event — add to your concrete subclass |

**Background tasks** — base class provides `id`, `type`, `payload`, `correlationId`, `causationId?`, `metadata?`. No `occurredAt`, `version`, or `source` in the base class — add them to `payload` or `metadata` if needed.

| Field | Type | Description |
|---|---|---|
| `id` | string | Unique task ID (UUID recommended) |
| `correlationId` | string | Originating operation identifier (UUID recommended) |
| `causationId` | string | Direct parent identifier (UUID recommended) |
| `type` | string | Task type discriminator |

**Structured log entries** should always include `correlationId` and `causationId` as indexed fields.
