...
- Throw a domain-specific exception extending `\DomainException` when an invariant is violated.
...
- Validate inside the constructor — throw a domain-specific exception extending `\DomainException` on invalid input.
...
- Payload may include immutable domain types such as value objects and identifiers, but should not expose live aggregates or entities.
...
- Keep payloads minimal and focused on the domain fact that occurred.
...
- Embed mutable entities or aggregate references in the payload.
...
- Return `null` from `findById` when the aggregate is not found — let the caller decide how to handle the missing result.
...
- The command bus catches `\DomainException` thrown by handlers and converts them to `Result::failed()`. Handlers never return `Result` directly.
...
**Don't**
- Throw `\DomainException` and catch it in the handler — let the bus handle the conversion.
...
**Do**
- Check `hasValue()` at the entry point before calling `value()`.
...
|---|---|---|
| `\DomainException` | Aggregate / domain service | Caught by command bus → `Result::failed()` |
...
**Key points**
- The entry point (controller) handles `Result` for domain failures from the command handler, and `ValidationErrors` for input failures. It never catches `\DomainException` from command handlers — those are wrapped by `RegistryCommandBus`. Exceptions thrown by domain event handlers propagate as infrastructure failures and are handled by the global error handler (5xx).
- Infrastructure exceptions bubble up to a global error handler.
- Never use exceptions for flow control within the domain — that is what `\DomainException` → `Result::failed()` is for.

**Do**
- Define a `\DomainException` subclass for each distinct business rule violation with a stable class name (the bus derives the error `code` from the class name automatically).

**Don't**
- Catch `\DomainException` in handlers.
...
sequenceDiagram
    ...
    Note over API: validate input (Command constructor)
...
sequenceDiagram
    ...
    Note over API: validate input (Query constructor)
    API->>Bus: ask(query)
...
- Handlers run within the same transaction as the command — if the transaction rolls back, their transactional effects are rolled back.
- The `DomainEventPublishingRepository` decorator publishes recorded events automatically after `save` to the configured `DomainEventBus`. When that bus is deferred, this means the events are buffered on `save` and the handlers execute later, after the command handler completes but before the transaction commits. Handlers are unaware of the publishing mechanism.
- The deferred bus (`DeferredDomainEventBus`) buffers events and dispatches them after the command handler completes but before the transaction commits. Handlers may therefore execute and still trigger rollback if they throw; keep side effects limited to transactional writes such as outbox inserts. Idempotent by event ID — saving the same aggregate twice in one transaction does not duplicate events.
...
- Read `correlationId` from `RequestContext` (set at the entry point) and set `causationId` to the triggering domain event's `id` — neither `Command` nor `DomainEvent` carry these fields.
...
**Do**
- Include `correlationId` (read from `RequestContext`) and `causationId` (the triggering domain event's `id`) in every task.
...
**Naming** — domain events and integration events use past tense and business language; background tasks typically use imperative, action-oriented business language. Every name must be meaningful to a non-technical stakeholder without explanation.
...
**Traceability** — `correlationId` does not live on `Command` or `DomainEvent`; it is held in a service-defined `RequestContext`, set at the entry point and read when constructing `IntegrationEvent` or `BackgroundTask`. Set `causationId` to the triggering domain event's `id`. These two fields, consistently propagated, make the causal chain reconstructable.
