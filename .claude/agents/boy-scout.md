...
- **Larger, cross-cutting, or touching a public contract**: do not execute it inline.
  Open a separate issue (in the requester's own `gh` identity, per `.claude/skills/gh-workflow/SKILL.md`)
...
- Every refactor commit uses `refactor:` and must **not** trigger a release — verify this
  is consistent with `.releaserc.json` / the commit-type table in `.claude/skills/gh-workflow/SKILL.md`.
